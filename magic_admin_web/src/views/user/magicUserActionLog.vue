  ...
  <div class="table-box">
    ...
    <div class="card table-main">
      ...
      <el-table ref="myTable" :data="tableData" border size="small">
        ...
        <el-table-column prop="action" label="操作类型" align="center" :min-width="150" />
        <el-table-column prop="afterValue" label="变更后值" align="center" :min-width="220" />
        <el-table-column prop="beforeValue" label="变更前值" align="center" :min-width="220" />
...
import { magicUserActionLogList } from '@/api/modules/user.js'
...
const getList = async () => {
  ...
  const res = await magicUserActionLogList(params)
  if (res.code === 200) {
...
const onResetSearch = () => {
  searchForm.value = { id: null, userId: null }
  timeValue.value = []
  getList()
}
...
const filesselectedfiles = async (str) => {
  ...
  try {
    const downFile = {
      fileName: 'magic_user_action_log',
