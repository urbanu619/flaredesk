  ...
  <div class="table-box">
    ...
    <div class="card table-main">
      ...
      <el-table ref="myTable" :data="tableData" border size="small">
        ...
        <el-table-column prop="productId" label="产品ID" align="center" :min-width="120" />
        ...
        <el-table-column prop="symbol" label="质押币种" align="center" :min-width="120" />
        <el-table-column prop="cumQuantity" label="累计质押数量" align="center" :min-width="150" />
        <el-table-column prop="quantity" label="当前质押数量" align="center" :min-width="150" />
        <el-table-column prop="cumUsdAmount" label="累计USD价值" align="center" :min-width="150" />
        <el-table-column prop="usdAmount" label="当前USD价值" align="center" :min-width="150" />
        <el-table-column prop="firstPledgeDate" label="首次质押日" align="center" :min-width="150" />
        <el-table-column prop="redeemedQuantity" label="累计释放数量" align="center" :min-width="150" />
        <el-table-column prop="redeemedUsdAmount" label="累计释放USD" align="center" :min-width="150" />
        <el-table-column prop="lastProfitPeriod" label="最近产出期号" align="center" :min-width="150" />
        <el-table-column prop="availableQuantity" label="待领取数量" align="center" :min-width="150" />
        <el-table-column prop="availableUsdAmount" label="待领取价值" align="center" :min-width="150" />
        <el-table-column prop="cumClaimQuantity" label="累计领取数量" align="center" :min-width="150" />
        <el-table-column prop="cumClaimUsdAmount" label="累计领取USD" align="center" :min-width="150" />
        <el-table-column prop="lastClaimPeriod" label="最近领取期号" align="center" :min-width="150" />
        <el-table-column prop="version" label="事务版本" align="center" :min-width="120" />
        <el-table-column prop="errDesc" label="错误信息" align="center" :min-width="220" />
...
import { magicStakeUserCurrentOrderList } from '@/api/modules/stake.js'
...
const getList = async () => {
  ...
  const res = await magicStakeUserCurrentOrderList(params)
  if (res.code === 200) {
...
const filesselectedfiles = async (str) => {
  ...
  const res = await magicStakeUserCurrentOrderList(params)
  try {
    await downLoadFile({ fileName: 'magic_stake_user_current_order', fileUrl: res.data.url })
