  ...
  <div class="table-box">
    ...
    <div class="card table-main">
      ...
      <el-table ref="myTable" :data="tableData" border size="small">
        ...
        <el-table-column prop="uid" label="交易所ID" align="center" :min-width="120" />
        <el-table-column prop="maxLevel" label="历史最高等级" align="center" :min-width="120" />
        <el-table-column prop="level" label="当前业绩等级" align="center" :min-width="120">
          <template #default="{ row }">{{ valueToLabel(levelListOption, row.level) }}</template>
        </el-table-column>
        <el-table-column prop="cumQuantity" label="累计奖励数量" align="center" :min-width="140" />
        <el-table-column prop="cumUsdAmount" label="累计奖励USD价值" align="center" :min-width="140" />
        <el-table-column prop="availableQuantity" label="待领取数量" align="center" :min-width="140" />
        <el-table-column prop="availableUsdAmount" label="待领取价值" align="center" :min-width="140" />
        <el-table-column prop="cumClaimQuantity" label="累计领取数量" align="center" :min-width="140" />
        <el-table-column prop="cumClaimUsdAmount" label="累计领取USD价值" align="center" :min-width="140" />
...
import { promotionUpgradeLevelList } from "@/api/modules/activity.js";
import { valueToLabel } from "@/utils/index";
import { levelListOption } from "@/utils/dict";
...
const getList = async () => {
  ...
  const res = await promotionUpgradeLevelList(params);
  if (res.code === 200) {
...
const filesselectedfiles = async str => {
  ...
  const res = await promotionUpgradeLevelList(params);
  try {
    await downLoadFile({ fileName: "promotion_upgrade_level", fileUrl: res.data.url });
