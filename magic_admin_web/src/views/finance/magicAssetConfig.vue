  ...
  <div class="table-box">
    <div class="card table-search" v-show="isShowSearch">
      <el-form ref="elSearchFormRef" :inline="true" size="small" :model="searchForm" @keyup.enter="onSubmit">
        ...
        <el-form-item label="币种">
          <el-input v-model="searchForm.symbol" placeholder="币种" style="width:139px" />
        </el-form-item>
    ...
    <div class="card table-main">
      ...
      <el-table ref="myTable" :data="tableData" border size="small">
        ...
        <el-table-column prop="withdrawMin" label="最小提现" align="center" :min-width="120" />
        <el-table-column prop="withdrawMax" label="最大提现" align="center" :min-width="120" />
        <el-table-column prop="withdrawFee" label="提现手续费" align="center" :min-width="120" />
        <el-table-column prop="enable" label="启用状态" align="center" :min-width="120" />
...
import { magicAssetConfigList } from '@/api/modules/finance.js'

const timeValue = ref([])
const isShowSearch = ref(true)
const searchForm = ref({ id: null, symbol: null })
const tableData = ref([])
...
const getList = async () => {
  ...
  const res = await magicAssetConfigList(params)
  if (res.code === 200) {
...
const onResetSearch = () => { searchForm.value = { id: null, symbol: null }; timeValue.value = []; getList() }
...
const filesselectedfiles = async (str) => {
  ...
  const res = await magicAssetConfigList(params)
  try {
    await downLoadFile({ fileName: 'magic_asset_config', fileUrl: res.data.url })
