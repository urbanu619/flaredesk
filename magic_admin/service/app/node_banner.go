...
type NodeBannerService struct {
	base.BizCommonService
}

func (s *NodeBannerService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewNodeBanner()).Get(c,s.DB())
}

func (s *NodeBannerService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    req := new(request[model.NodeBanner])
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.NodeBanner{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.NodeBanner](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewNodeBanner()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *NodeBannerService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewNodeBanner().TableName())
	if err != nil {
