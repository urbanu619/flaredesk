...
func (s *SysI18nService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewSysI18n()).Get(c,s.DB())
}

func (s *SysI18nService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.SysI18n{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.SysI18n](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewSysI18n()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *SysI18nService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewSysI18n().TableName())
	if err != nil {
