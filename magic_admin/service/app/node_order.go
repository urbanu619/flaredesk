...
type NodeOrderService struct {
	base.BizCommonService
}

func (s *NodeOrderService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewNodeOrder()).Get(c,s.DB())
}

func (s *NodeOrderService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    req := new(request[model.NodeOrder])
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.NodeOrder{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.NodeOrder](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewNodeOrder()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *NodeOrderService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewNodeOrder().TableName())
	if err != nil {
