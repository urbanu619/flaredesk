...
type MagicAssetRwRecordService struct {
	base.BizCommonService
}

func (s *MagicAssetRwRecordService) Get(c *gin.Context) {
	s.SetDbAlias("app")
	base.NewBaseHandler(model.NewMagicAssetRwRecord()).Get(c, s.DB())
}

func (s *MagicAssetRwRecordService) Find(c *gin.Context) {
	s.SetDbAlias("app")
	...
	req := new(request[model.MagicAssetRwRecord])
	...
	var url string
	colInfo := s.GetColumnCommentFromStruct(model.MagicAssetRwRecord{})
	var err error
	if req.IsExport != nil && *req.IsExport {
		...
		url, err = base.ExportCsv[model.MagicAssetRwRecord](db, fields, colInfo)
		if err != nil {
			response.Resp(c, err.Error())
			return
		}
	}
	resp, err := base.NewQueryBaseHandler(model.NewMagicAssetRwRecord()).List(db, req)
	if err != nil {
		response.Resp(c, err.Error())
		return
	}
...
func (s *MagicAssetRwRecordService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewMagicAssetRwRecord().TableName())
	if err != nil {
