...
type SysCoinService struct {
	base.BizCommonService
}

func (s *SysCoinService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewSysCoin()).Get(c,s.DB())
}

func (s *SysCoinService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.SysCoin{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.SysCoin](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewSysCoin()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *SysCoinService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewSysCoin().TableName())
	if err != nil {
