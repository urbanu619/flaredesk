...
type SysLevelConfigService struct {
	base.BizCommonService
}

func (s *SysLevelConfigService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewSysLevelConfig()).Get(c,s.DB())
}

func (s *SysLevelConfigService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    req := new(request[model.SysLevelConfig])
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.SysLevelConfig{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.SysLevelConfig](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewSysLevelConfig()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *SysLevelConfigService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewSysLevelConfig().TableName())
	if err != nil {
