...
type SysSignConfigService struct {
	base.BizCommonService
}

func (s *SysSignConfigService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewSysSignConfig()).Get(c,s.DB())
}

func (s *SysSignConfigService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    req := new(request[model.SysSignConfig])
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.SysSignConfig{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.SysSignConfig](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewSysSignConfig()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *SysSignConfigService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewSysSignConfig().TableName())
	if err != nil {
