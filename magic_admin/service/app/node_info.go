...
func (s *NodeInfoService) Get(c *gin.Context) {
    s.SetDbAlias("app")
    base.NewBaseHandler(model.NewNodeInfo()).Get(c,s.DB())
}

func (s *NodeInfoService) Find(c *gin.Context) {
    s.SetDbAlias("app")
    ...
    req := new(request[model.NodeInfo])
    ...
    var url string
    colInfo := s.GetColumnCommentFromStruct(model.NodeInfo{})
    var err error
    if req.IsExport != nil && *req.IsExport {
    	...
    	url, err = base.ExportCsv[model.NodeInfo](db, fields, colInfo)
    	if err != nil {
    		response.Resp(c, err.Error())
    		return
    	}
    }
    resp, err := base.NewQueryBaseHandler(model.NewNodeInfo()).List(db, req)
    if err != nil {
    	response.Resp(c, err.Error())
    	return
    }
...
func (s *NodeInfoService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewNodeInfo().TableName())
	if err != nil {
