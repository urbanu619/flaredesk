...
type MagicUserQuotaService struct {
	base.BizCommonService
}

func (s *MagicUserQuotaService) Get(c *gin.Context) {
	s.SetDbAlias("app")
	base.NewBaseHandler(model.NewMagicUserQuota()).Get(c, s.DB())
}

func (s *MagicUserQuotaService) Find(c *gin.Context) {
	s.SetDbAlias("app")
	...
	req := new(request[model.MagicUserQuota])
	...
	var url string
	colInfo := s.GetColumnCommentFromStruct(model.MagicUserQuota{})
	var err error
	if req.IsExport != nil && *req.IsExport {
		...
		url, err = base.ExportCsv[model.MagicUserQuota](db, fields, colInfo)
		if err != nil {
			response.Resp(c, err.Error())
			return
		}
	}
	resp, err := base.NewQueryBaseHandler(model.NewMagicUserQuota()).List(db, req)
	if err != nil {
		response.Resp(c, err.Error())
		return
	}
...
func (s *MagicUserQuotaService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewMagicUserQuota().TableName())
	if err != nil {
