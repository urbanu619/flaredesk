...
type MagicUserLogsService struct {
	base.BizCommonService
}

func (s *MagicUserLogsService) Get(c *gin.Context) {
	s.SetDbAlias("app")
	base.NewBaseHandler(model.NewMagicUserLogs()).Get(c, s.DB())
}

func (s *MagicUserLogsService) Find(c *gin.Context) {
	s.SetDbAlias("app")
	...
	req := new(request[model.MagicUserLogs])
	...
	var url string
	colInfo := s.GetColumnCommentFromStruct(model.MagicUserLogs{})
	var err error
	if req.IsExport != nil && *req.IsExport {
		...
		url, err = base.ExportCsv[model.MagicUserLogs](db, fields, colInfo)
		if err != nil {
			response.Resp(c, err.Error())
			return
		}
	}
	resp, err := base.NewQueryBaseHandler(model.NewMagicUserLogs()).List(db, req)
	if err != nil {
		response.Resp(c, err.Error())
		return
	}
...
func (s *MagicUserLogsService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewMagicUserLogs().TableName())
	if err != nil {
