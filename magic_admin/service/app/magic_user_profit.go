...
type MagicUserProfitService struct {
	base.BizCommonService
}

func (s *MagicUserProfitService) Get(c *gin.Context) {
	s.SetDbAlias("app")
	base.NewBaseHandler(model.NewMagicUserProfit()).Get(c, s.DB())
}

func (s *MagicUserProfitService) Find(c *gin.Context) {
	s.SetDbAlias("app")
	...
	req := new(request[model.MagicUserProfit])
	...
	var url string
	colInfo := s.GetColumnCommentFromStruct(model.MagicUserProfit{})
	var err error
	if req.IsExport != nil && *req.IsExport {
		...
		url, err = base.ExportCsv[model.MagicUserProfit](db, fields, colInfo)
		if err != nil {
			response.Resp(c, err.Error())
			return
		}
	}
	resp, err := base.NewQueryBaseHandler(model.NewMagicUserProfit()).List(db, req)
	if err != nil {
		response.Resp(c, err.Error())
		return
	}
...
func (s *MagicUserProfitService) Comment(c *gin.Context) {
	s.SetDbAlias("app")
	dbs, err := s.GetColumnComment("app", model.NewMagicUserProfit().TableName())
	if err != nil {
